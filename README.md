...
## 🥗 Nutfruit Recetas — Sistema de videos YouTube

Este repositorio incluye un sistema completo y listo para producir videos de recetas para el canal **Nutfruit Recetas**. Los componentes visuales, la estructura de timing y el estilo de marca ya están construidos — solo necesitas poner tus assets y pedir el video.
...
Requisitos: **Node.js 20+** y **Claude Code** instalado.

### Agregar tus assets

Crea una carpeta para tu receta dentro de `public/assets/nutfruit-recetas/`:

```
public/assets/nutfruit-recetas/
└── mi-recetayt/
    ├── video_16x9.mp4       ← video de cocina (1920×1080)
    ├── musica video X.mp3   ← música de fondo
    ├── whoosh-in.mp3
    ├── whoosh-out.mp3
    ├── intro.mp3
    ├── ingredientes.mp3
    ├── pasos.mp3            ← o paso1.mp3, paso2.mp3... si son separados
    └── final.mp3
```

> ⚠️ Los archivos `.mp3` y `.mp4` **no se suben al repositorio** (están en `.gitignore`).
> Cada miembro del equipo los coloca en su propia máquina.

### Crear un video nuevo

Abre Claude Code en la carpeta del proyecto y describe la receta:

> "Crea el video de YouTube para la receta de **Brownie de Nueces**. El video está en `public/assets/nutfruit-recetas/brownieyt/brownie_16x9.mp4` y dura 52 segundos. Los ingredientes son: nueces 200g, mantequilla 80g, azúcar 120g, huevos 3, harina 160g, cacao 40g. Los pasos son: [...]"

Claude seguirá automáticamente el estilo Nutfruit: mismo diseño, mismos colores, mismas animaciones.

### Actualizar cuando alguien del equipo hace cambios en el código

```bash
git pull
# No necesitas reinstalar a menos que cambie package.json
```

### Recetas ya producidas

| Video | Composición | Duración aprox. |
|---|---|---|
| Chilaquiles de Ciruela y Pistachos | `ChilaquilesYT` | 49s |
| Veggie Flautas con Arándanos | `FlautasYT` | 63s |
| Capirotada Mexicana de Frutos Secos | `CapirotadaYT` | 55s |
| Torta Tres Leches de Almendras | `TresLechesYT` | 68s |

### Renderizar a MP4

```bash
npx remotion render ChilaquilesYT out/chilaquiles.mp4
npx remotion render FlautasYT out/flautas.mp4
npx remotion render CapirotadaYT out/capirotada.mp4
npx remotion render TresLechesYT out/tresleches.mp4
```
